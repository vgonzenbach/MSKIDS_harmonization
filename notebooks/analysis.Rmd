...
dict_df <- readxl::read_excel('data/MUSE_ROI_Dict.xlsx') %>% 
  ...
  mutate(ROI_NAME = str_replace(ROI_NAME, "( AnG| Calc| FRP| FuG| IOG| MCgG| PrG| PO)", "") %>%
           ...
           str_replace_all("Wm", "White Matter") %>% 
           str_to_title
         )
  
# to exclude abbreviation in significant rois

# to exclude all abbreviations from ROI_NAME (currently ails)
...
This analysis shows that MS is associated with specific brain volume changes in a pediatric population.
...
Model equation:
\begin{align}
ROI &= \beta_0 + \beta_1*Sex + \beta_2*Age + \beta_3*Age^2 + \beta_4*Sex*Age + \beta_5*ICV + \beta_6*MS + \epsilon
\end{align}

#### Significant main effects: MS 

With healthy controls as the reference group, the main effect in milliliters ($mL$) of having MS, after accounting for control variables, is shown in the `estimate` column for each significant ROI in the table below. Individual effects are described below the table.
...
For our secondary analysis, we ran models that included an interaction term.

Model equation:

\begin{align}
ROI &= \beta_0 + \beta_1*Sex + \beta_2*Age + \beta_3*Age^2 + \beta_4*Sex*Age + \beta_5*ICV + \beta_6*MS*Sex + \epsilon
\end{align}
...
sig_interaction_effects_df <- models %>% 
  filter(term %in% c('SexMALE:MS1')) %>% 
  mutate(p.value.adj = p.adjust(p.value, method='fdr'),4) %>% 
  filter(p.value.adj < 0.05) %>% 
  mutate(p.value = round(p.value, 4),
         p.value.adj = round(p.value.adj, 4)) %>% 
  arrange(by = TISSUE_SEG)
...
describe_interaction_effect <- function(row){
  sprintf("The %s volume of MS males is %.3f mL %s, on average, compared to that of MS females after accounting for the main effect of MS (95%% CI: (%.3f, %.3f), p = %.3f).",
         row$ROI_NAME, 
         round(abs(row$estimate), 2), 
         ifelse(sign(row$estimate) == 1, 'bigger', 'smaller'), 
         round(row$estimate - row$std.error, 3), 
         round(row$estimate + row$std.error, 3), 
         round(row$p.value.adj, 3)
        ) |> invisible()
}
...
Here, we regress out the effect of Age, Age Squared and ICV to better visualize the effect of the MS and sex interaction.  
