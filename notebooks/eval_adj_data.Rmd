...
MS (count by site)
```{r}
MS_adj %>% group_by(site) %>% count() %>% pander()
```

MS (count by sex)
```{r}
MS_adj %>% group_by(sex) %>% count(site) %>% pander()
```
...
```{r}
HC_raw %>% group_by(site) %>% count(sex) %>% kable()
```
...
Count per site: 
```{r}
rbind(HC_adj, MS_adj) %>% group_by(site) %>% count() %>% kable()
```

Count by sex and site:
```{r}
rbind(HC_adj, MS_adj) %>% group_by(site) %>% count(sex) %>% kable()
```
...
```{r}
MS_raw %>% ggplot(aes(x=site, col=site, y=age)) + geom_boxplot() + facet_wrap(~sex) + xlab("Site") + ylab("Age") + ggtitle("Age distribution of MS patients across sites")
```

## Age across sites [HC]

```{r}
HC_raw %>% ggplot(aes(x=site, col=site, y=age)) + geom_boxplot() + facet_wrap(~sex) + xlab("Site") + ylab("Age") + ggtitle("Age distribution of Healthy Controls across sites")
```
...
```{r}
# Full model
check_roi_site_fx(MS_raw, roi_small) %>% pander()
```

Sex not considered:
```{r}
# Without considering sex
check_roi_site_fx(MS_raw, roi_small, mod_sex = FALSE) %>% pander()
```
...
```{r}
# Full model
check_roi_site_fx(MS_adj, roi_small) %>% pander()
```
...
```{r}
# Without considering sex
check_roi_site_fx(MS_adj, roi_small, mod_sex = FALSE) %>% pander()
```

## Site effects by sex: MS adj

Females: 

```{r}
filter(MS_adj, sex == "FEMALE") %>%  check_roi_site_fx(roi_small, mod_sex = FALSE) %>% pander()
```

Males:

```{r}
filter(MS_adj, sex == "MALE") %>%  check_roi_site_fx(roi_small, mod_sex = FALSE) %>% pander()
```
...
```{r}
# Full model
check_roi_site_fx(HC_raw, roi_small) %>% pander()
```
...
```{r}
# Without considering sex
check_roi_site_fx(HC_raw, roi_small, mod_sex = FALSE) %>% pander()
```
...
```{r}
# Full model
check_roi_site_fx(HC_adj, roi_small) %>% pander()
```

Sex not considered:
```{r}
# Without considering sex
check_roi_site_fx(HC_adj, roi_small, mod_sex = FALSE) %>% pander()
```

## Site effects by sex: HC adj

Females:

```{r}
filter(HC_adj, sex == "FEMALE") %>%  check_roi_site_fx(roi_small, mod_sex = FALSE) %>% pander()
```

Males:

```{r}
filter(HC_adj, sex == "MALE") %>%  check_roi_site_fx(roi_small, mod_sex = FALSE) %>% pander()
```
