...
par(mfrow= c(1, 2))
plot(gam_icv_F, residuals = TRUE, pch = 1, main = "Females", ylab = "raw ICV", cex = 0.75, shade = TRUE, seWithMean = TRUE, shift = coef(gam_icv_M)[1])
plot(gam_icv_M, residuals = TRUE, pch = 1, cex = 0.75, ylab = "raw ICV", shade = TRUE, seWithMean = TRUE, shift = coef(gam_icv_F)[1], main = 'Males')
```
...
gam_vn = mgcv::gam(CSF ~ s(age,  by = sex, bs = 'tp') + sex, 
                    data = pnc,
                    family = gaussian,
                    method = 'REML')
summary(gam_vn)
...
# Label CSF
pnc_M$CSF_std = set_label(pnc_M$CSF_std, label = "CSF (Males)")
pnc_F$CSF_std = set_label(pnc_F$CSF_std, label = "CSF (Females)")
